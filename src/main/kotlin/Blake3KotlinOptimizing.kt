...
class Blake3KotlinOptimizing {
    ...
    private class Node(
        ...
        fun rootOutputBytes(outLen: Int): IntArray {
            ...
            while (outputCounter < outputsNeeded) {
                val words = compress(
                    inputChainingValue,
                    blockWords, outputCounter.toLong(),
    ...
    private class ChunkState(var chainingValue: IntArray, var chunkCounter: Long, var flags: Int) {
        ...
        fun createNode(): Node {
            return Node(
                chainingValue, wordsFromLEBytes(block), chunkCounter,
                blockLen, flags or startFlag() or CHUNK_END
            )
        }
    }
    ...
    fun update(file: File) {
        ...
        FileInputStream(file).use { ios ->
            ...
            var read = 0
            while (ios.read(intArrayToByteArray(buffer)).also { read = it } != -1) {
                if (read == buffer.size) {
                    update(buffer)
                } else {
    ...
    fun update(input: ByteArray) {
        update(byteArrayToIntArray(input))
    }
    /**
     ...
     */
    @JvmOverloads
    fun digest(hashLen: Int = DEFAULT_HASH_LEN): IntArray {
    ...
    companion object {
        ...
        private fun wordsFromLEBytes(bytes: IntArray): IntArray {
            ...
            val buf = ByteBuffer.wrap(intArrayToByteArray(bytes)).order(ByteOrder.LITTLE_ENDIAN)
        ...
        private fun intArrayToByteArray(intArray:IntArray):ByteArray{
            ...
            intArray.mapIndexed { index, byte ->
